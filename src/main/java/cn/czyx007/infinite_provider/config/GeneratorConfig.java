...
public class GeneratorConfig {
    ...
    @Config.Comment({"基础发电配置", "Basic Power Generation Settings"})
    public static BasicPower basicPower = new BasicPower();
    
    public static class BasicPower {
        @Config.Comment({"1个水供应器的基础发电量 [默认: 256 RF/t]", "Base power generation for 1 water provider [Default: 256 RF/t]"})
        @Config.RangeInt(min = 0)
        public int powerLevel1 = 256;
        
        @Config.Comment({"2个水供应器的基础发电量 [默认: 512 RF/t]", "Base power generation for 2 water providers [Default: 512 RF/t]"})
        @Config.RangeInt(min = 0)
        public int powerLevel2 = 512;
        
        @Config.Comment({"3个水供应器的基础发电量 [默认: 768 RF/t]", "Base power generation for 3 water providers [Default: 768 RF/t]"})
        @Config.RangeInt(min = 0)
        public int powerLevel3 = 768;
        
        @Config.Comment({"4个水供应器的基础发电量 [默认: 1024 RF/t]", "Base power generation for 4 water providers [Default: 1024 RF/t]"})
        @Config.RangeInt(min = 0)
    ...
    @Config.Comment({"岩浆集群加成配置", "Lava Cluster Bonus Settings"})
    public static LavaClusterBonus lavaClusterBonus = new LavaClusterBonus();
    
    public static class LavaClusterBonus {
        @Config.Comment({"单个岩浆供应器的加成倍数 [默认: 1.0]", "Bonus multiplier for single lava provider [Default: 1.0]"})
        @Config.RangeDouble(min = 0.1, max = 10.0)
        public double singleBonus = 1.0;
        
        @Config.Comment({"十字岩浆集群的加成倍数 [默认: 2.0]", "Bonus multiplier for cross lava cluster [Default: 2.0]"})
        @Config.RangeDouble(min = 0.1, max = 10.0)
        public double crossBonus = 2.0;
        
        @Config.Comment({"3x3岩浆集群的加成倍数 [默认: 4.0]", "Bonus multiplier for 3x3 lava cluster [Default: 4.0]"})
        @Config.RangeDouble(min = 0.1, max = 10.0)
        public double clusterBonus = 4.0;
        
        @Config.Comment({"是否启用岩浆集群检测 [默认: true]", "Enable lava cluster detection [Default: true]"})
        public boolean enableClusterDetection = true;
    }
    
    // ============ 能量存储配置 ============
    
    @Config.Comment({"能量存储配置", "Energy Storage Settings"})
    public static EnergyStorage energyStorage = new EnergyStorage();
    
    public static class EnergyStorage {
        @Config.Comment({"水供应器的能量存储容量 [默认: 2.1G RF]", "Energy storage capacity of water provider [Default: 2.1G RF]"})
        @Config.RangeInt(min = 0)
        public int waterProviderCapacity = Integer.MAX_VALUE;
        
        @Config.Comment({"水供应器的最大能量输出速率 [默认: 2.1G RF/t]", "Maximum energy output rate of water provider [Default: 2.1G RF/t]"})
        @Config.RangeInt(min = 0)
